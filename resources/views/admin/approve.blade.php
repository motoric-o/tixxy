...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
    ...
    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider
        ...
        @else bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800/50
        @endif">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-6">
                @if($order->payment_proof)
                    <div class="rounded-xl overflow-hidden border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-900">
                        <img src="{{ asset('storage/' . $order->payment_proof) }}"
                            alt="Payment proof for Order #{{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }}"
                            class="w-full h-auto max-h-[600px] object-contain">
                    </div>
                @else
                    <div class="rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-600 p-12 text-center">
                        ...
                        <p class="text-xs text-gray-300 dark:text-gray-600 mt-1">The user has not submitted any payment evidence yet.</p>
                    </div>
                @endif
            </div>
                ...
                <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <form action="/manage/orders/{{ $order->id }}/decline" method="POST" class="flex-1"
                            onsubmit="return confirm('Decline this payment? This will remove the payment proof and reset the order to pending status.')">
                            @csrf
                            <button type="submit"
            ...
            @elseif($order->status === 'completed')
                <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700">
                    <div class="flex items-center gap-3 p-4 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/50">
                        <div class="w-10 h-10 rounded-xl bg-green-100 dark:bg-green-900/50 flex items-center justify-center text-green-600 dark:text-green-400 shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        ...
                        <div>
                            ...
                            <p class="text-xs text-green-600 dark:text-green-400">This order has been completed and the confirmation email was sent.</p>
                        </div>
                    </div>
                </div>
            @endif
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-sm font-bold text-gray-800 dark:text-white">#{{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }}</span>
                </div>
                <div class="flex justify-between items-center">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-sm font-medium text-gray-800 dark:text-white text-right max-w-[60%] truncate">{{ $order->event->title ?? 'N/A' }}</span>
                </div>
                    ...
                    @foreach($grouped as $eventTicketTypeId => $details)
                        ...
                        <div class="flex justify-between items-center py-2">
                            <div>
                                ...
                                <p class="text-xs text-gray-400">{{ $quantity }}x @ Rp {{ number_format($price, 0, ',', '.') }}</p>
                            </div>
                            <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">Rp {{ number_format($price * $quantity, 0, ',', '.') }}</span>
                        </div>
                    @endforeach
                </div>
                <div class="border-t border-gray-100 dark:border-gray-700 pt-4 flex justify-between items-center">
                    ...
                    <span class="text-lg font-extrabold text-indigo-600 dark:text-indigo-400">Rp {{ number_format($order->amount, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-indigo-600 dark:text-indigo-400">{{ $order->user->email ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div class="flex items-start gap-3">
                    <div class="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z">
                            </path>
