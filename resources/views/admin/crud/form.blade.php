...
@section('content')
    ...
    <div class="w-full flex justify-start">
        <div
            class="grid grid-cols-1 {{ !empty($detailFields ?? []) ? 'lg:grid-cols-3' : '' }} gap-6 {{ $method === 'POST' ? 'w-2/5' : 'w-3/5' }}">
                                ...
                                <div class="space-y-4">
                                    @foreach ($standardFields as $detail)
                                        <div class="flex justify-between items-center group">
                                            <span
                                                class="text-sm text-gray-600 dark:text-gray-400">{{ $detail['label'] }}</span>
                                            @if (isset($detail['url']))
                                                <a href="{{ $detail['url'] }}"
                                                    class="text-sm font-semibold text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 hover:underline transition-colors duration-300">
                                                    {{ $detail['value'] }}
                                                </a>
                                            @else
                                                <span
                                                    class="text-sm font-semibold text-gray-900 dark:text-white">{{ $detail['value'] }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    @foreach ($detailFields as $detail)
                        @if (($detail['type'] ?? null) === 'table' && !empty($detail['value']))
                            <div
                                class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                                <div
                                    class="p-6 border-b border-gray-200 dark:border-gray-700 dark:bg-gray-900 rounded-t-xl">
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $detail['label'] }}
                                    </h3>
                                </div>
                                <div class="p-6">
                        ...
                        @if (($detail['type'] ?? null) === 'badges' && !empty($detail['value']))
                            <div
                                class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                                <div
                                    class="p-6 border-b border-gray-200 dark:border-gray-700 dark:bg-gray-900 rounded-t-xl flex items-center justify-between">
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $detail['label'] }}
                                    </h3>
                                </div>
                                <div class="p-6">
                                    <div class="flex flex-wrap gap-2">
                                        @foreach ($detail['value'] as $badge)
                                            @if (isset($badge['url']))
                                                <a href="{{ $badge['url'] }}"
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all duration-300 hover:scale-105 active:scale-95
                            {{ ($badge['color'] ?? 'gray') === 'green'
                                ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-900/50'
                                : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                                                    <span
                                                        class="w-1.5 h-1.5 rounded-full {{ ($badge['color'] ?? 'gray') === 'green' ? 'bg-green-500' : 'bg-gray-400' }}"></span>
            ...
            <div class="{{ !empty($detailFields ?? []) ? 'lg:col-span-2' : 'max-w-4xl w-full mx-auto' }}">
                        ...
                        <form action="{{ $action }}" method="POST">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                                @foreach ($fields as $field)
                                    <div
                                        class="{{ ($field['type'] ?? 'text') === 'textarea' || ($field['wide'] ?? false) ? 'md:col-span-2' : '' }}">
                            ...
                            <div class="flex items-center gap-3 mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                                <button type="submit"
                                    class="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white rounded-lg bg-gradient-to-r from-[#4a00e0] via-[#8e2de2] to-[#4a00e0] bg-[length:200%_auto] hover:bg-[position:right_center] transition-all duration-300 shadow-md hover:shadow-[0_0_15px_rgba(168,85,247,0.4)]">
                                ...
                                <a href="{{ $backUrl }}"
                                    class="px-6 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-300">
                                    Cancel
                                </a>
                            </div>
                        </form>
