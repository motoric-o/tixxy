    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <form action="" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
            ...
            <div class="lg:col-span-1 flex items-end gap-2">
                <div class="flex-grow">
                    ...
                    <select name="sort"
                        ...
                        <option value="amount_desc" {{ request('sort') == 'amount_desc' ? 'selected' : '' }}>Amount (High to
                            Low)</option>
                        <option value="amount_asc" {{ request('sort') == 'amount_asc' ? 'selected' : '' }}>Amount (Low to
                            High)</option>
                        <option value="customer_asc" {{ request('sort') == 'customer_asc' ? 'selected' : '' }}>Customer (A-Z)
                        </option>
                        <option value="customer_desc" {{ request('sort') == 'customer_desc' ? 'selected' : '' }}>Customer
                            (Z-A)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="lg:col-span-6 flex justify-end gap-2 mt-2">
                <a href="/manage/manage-event-orders/{{ $event->id }}"
                    class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700 transition duration-200 shadow-sm">
                    Clear Filters
                </a>
                <button type="submit"
                    class="px-4 py-2 text-sm font-semibold text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors duration-200 shadow-sm">
                    Apply Filters
                </button>
            </div>
        </form>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700 text-gray-900 dark:text-gray-200">
                    @forelse ($orders as $order)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-200 cursor-pointer"
                            onclick="window.location='/manage/orders/{{ $order->id }}/approve'">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($order->payment_proof)
                                    <svg class="inline w-4 h-4 text-blue-500 ml-1" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24" title="Payment Proof Uploaded">
