        ...
        <div class="mb-6 flex flex-row justify-between items-center">
            ...
            <div class="flex flex-col items-end gap-1">
                <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Assigned
                    Organizer</label>
                <select x-model="formData.user_id" @change="saveOrganizer()"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div
                        ...
                        <button type="button" @click="if(editing) { save() } else { editing = true }"
                            class="transition-all duration-300 disabled:opacity-50" :disabled="saving"
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Event
                                    Name</label>
                                <p x-show="!editing" class="text-gray-900 dark:text-white font-medium"
                                    x-text="formData.title">{{ $item->title }}</p>
                                <input x-show="editing" type="text" x-model="formData.title"
                            ...
                            <div>
                                ...
                                <p x-show="!editing" class="text-gray-900 dark:text-white" x-text="categoryName">
                                    {{ $item?->category?->name ?? 'None' }}</p>
                                <select x-show="editing" x-model="formData.category_id"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300"
                                    style="display: none;">
                                    <option value="">Select Category</option>
                                    @foreach($categories as $id => $name)
                                        <option value="{{ $id }}" @if($item->category_id == $id) selected @endif>{{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Start
                                        Time</label>
                                    <p x-show="!editing" class="text-gray-900 dark:text-white"
                                        x-text="formatDate(formData.start_time)"></p>
                                    <input x-show="editing" type="datetime-local" x-model="formData.start_time"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300"
                                        style="display: none;">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End
                                        Time</label>
                                    <p x-show="!editing" class="text-gray-900 dark:text-white"
                                        x-text="formatDate(formData.end_time)"></p>
                                    <input x-show="editing" type="datetime-local" x-model="formData.end_time"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300"
                                        style="display: none;">
                                </div>
                            </div>
                            <div>
                                ...
                                <p x-show="!editing" class="text-gray-900 dark:text-white" x-text="formData.location">
                                    {{ $item->location }}</p>
                                <input x-show="editing" type="text" x-model="formData.location"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300"
                                    style="display: none;">
                            </div>
                            <div>
                                ...
                                <p x-show="!editing" class="text-gray-900 dark:text-white whitespace-pre-wrap"
                                    x-text="formData.description">{{ $item->description }}</p>
                                <textarea x-show="editing" rows="4" x-model="formData.description"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300"
                                    style="display: none;">{{ $item->description }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="status" x-model="formData.status" @change="save()"
                                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-600 focus:border-purple-600 dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300">
                            ...
                            <div>
                                ...
                                <input type="number" name="quota" x-model.number="formData.quota"
                                    @change.debounce.500ms="save()"
                                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-600 focus:border-purple-600 dark:focus:ring-purple-500 dark:focus:border-purple-500 transition-colors duration-300">
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-xs px-2.5 py-1 rounded-md transition-colors duration-300"
                                :class="totalTicketCapacity > formData.quota ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800' : 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 border border-purple-200 dark:border-purple-800'">
                                <span class="font-bold" x-text="totalTicketCapacity"></span> / <span
                                    x-text="formData.quota"></span> Quota
                            </div>
                        </div>
                        <button type="button" @click="addTicketType()"
                            class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 disabled:opacity-50 disabled:cursor-not-allowed"
                            :disabled="totalTicketCapacity >= formData.quota">
                            + Add Ticket Type
                        </button>
                    </div>
                    <div class="p-6">
                                ...
                                <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                                    ...
                                    <div x-show="!ticket.editMode">
                                        <div class="flex justify-between items-start">
                                            <div>
                                                <h4 class="font-semibold text-gray-900 dark:text-white"
                                                    x-text="getTicketTypeName(ticket.ticket_type_id) || 'Unknown Type'">
                                                </h4>
                                                <p class="text-sm text-gray-500 dark:text-gray-400">Ticket configuration</p>
                                            </div>
                                            <div class="text-right">
                                                <p class="font-semibold text-gray-900 dark:text-white"
                                                    x-text="'Rp ' + parseFloat(ticket.price).toLocaleString('id-ID')"></p>
                                        ...
                                        <div class="mt-3 grid grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-400">
                                            <div><span class="font-medium">Capacity:</span> <span
                                                    x-text="ticket.capacity"></span></div>
                                        </div>
                                        <div class="mt-3 flex gap-2 justify-end">
                                            <button type="button" @click="ticket.editMode = true"
                                                class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700">Edit</button>
                                            <button type="button" @click="removeTicketType(index)"
                                                class="text-xs font-medium text-red-500 dark:text-red-400 hover:text-red-700">Remove</button>
                                        </div>
                                    </div>
                                    {{-- Edit mode --}}
                                    <div x-show="ticket.editMode" style="display:none;">
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Ticket
                                                    Type</label>
                                                <select x-model="ticket.ticket_type_id"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Price
                                                    (Rp)</label>
                                                <input type="number" step="0.01" x-model="ticket.price" placeholder="0"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                                            </div>
                                            <div>
                                                <label
                                                    class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Capacity</label>
                                                <input type="number" x-model.number="ticket.capacity" placeholder="0"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                                            </div>
                                        </div>
                                        <div class="flex gap-2 justify-end items-center">
                                            <span x-show="totalTicketCapacity > formData.quota"
                                                class="text-xs text-red-500 dark:text-red-400 mr-2">Exceeds total event
                                                quota!</span>
                                            <button type="button" :disabled="totalTicketCapacity > formData.quota"
                                                @click="if(totalTicketCapacity <= formData.quota) { ticket.editMode = false; saveTicketTypes(); }"
                                                class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-white rounded-lg bg-gradient-to-r from-[#4a00e0] via-[#8e2de2] to-[#4a00e0] bg-[length:200%_auto] hover:bg-[position:right_center] shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                                            ...
                                            <button type="button" @click="cancelEdit(ticket, index)"
                                                class="text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 px-3 py-1.5">Cancel</button>
                                        </div>
                                    </div>
                                </div>
                            </template>
                            <div x-show="formData.ticket_types.length === 0"
